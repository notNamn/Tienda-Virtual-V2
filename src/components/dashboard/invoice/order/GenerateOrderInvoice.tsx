...
import { addMultipleProductToInvoice, createInvoiceOrder, processInvoiceOrder } from '@/service/InvoiceService';
...
export default function GenerateOrderInvoice() {
    // obtener el carrio de compras del store 
    ...
    const handleGenerateOrden = async () => {
        try {
            ...
            // 1. Crear la orden
            const newOrder = await createInvoiceOrder();
            if (!newOrder) {
            ...
            const result = await addMultipleProductToInvoice(orderItems);
            ...
            const sale = await processInvoiceOrder(customerData);
            ...
            if (sale) {
                ...
                route.push(`/dashboard/invoices/view/${sale.id}`);
            } else {
